namespace :db do
	...
	task dump: :environment do
	    ...
	    with_config do |app, host, database, user, password|
			...
			exec cmd
			puts "Database Dump(#{DateTime.now})"
			Rails.logger.info "Database Dump(#{DateTime.now})"
		rescue => e
			puts "Database Dump(#{DateTime.now}): #{e}"
			Rails.logger.info "Database Dump(#{DateTime.now}): #{e}"
		end
	end
  ...
  task restore: :environment do
	    ...
	    exec cmd
			puts "Restore Dump(#{DateTime.now})"
			Rails.logger.info "Restore Dump(#{DateTime.now})"
		rescue => e
			puts "Restore Dump(#{DateTime.now}): #{e}"
			Rails.logger.info "Restore Dump(#{DateTime.now}): #{e}"
		end
	end
